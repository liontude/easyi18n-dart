...
| --- | --- | --- |
| [`easyi18n`](easyi18n_flutter) | `easyi18n` | Runtime Flutter SDK — source-as-key `tr()`, an offline floor from baked assets, and live hot-updates from the delivery manifest/CDN. No codegen, no rebuild to change a value. |
...
> Folder names mirror the internal package ids (`easyi18n_flutter` publishes as
> `easyi18n`). Pick the flow that fits: the **runtime SDK** for hot-updating
> strings, or the **CLI** to stay on stock `flutter_localizations` + `gen-l10n`.
